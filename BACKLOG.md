...
## Ground-truth leaks in the analysis datasets (found 2026-08-26; BLOCKING two notebooks)

The Data-analysis notebooks are supposed to show that a real tool can recover something from iscc
output. Two of them are currently handed the answer as input, which makes the recovery vacuous.

- **NOW — TreeMHN's trees are the true trees.** `_treemhn` in `validation/make_analysis_data.py`
  calls `ig.to_treemhn_trees(tumors)`, and `to_mutation_tree` builds each patient's tree from the
  realised clone `event_order` — i.e. iscc's own clone structure. TreeMHN is therefore estimating
  rates from the topology it was meant to infer. **Fix (user direction 2026-08-26): the trees must
  come from a tree-inference tool.** SCITE is the right one — its input is a noisy mutation x cell
  binary matrix (`3` = missing, `-fd`/`-ad` for FP/dropout) and its output is a mutation tree, which
  is TreeMHN's input format. PyClone-VI is the wrong shape: it yields mutation clusters and CCFs,
  not a tree, and would need a tree-builder bolted on. SCITE is a dependency-free single-command
  build (`clang++ *.cpp -o scite`, cbg-ethz/SCITE) — needs an `iscc-scite` env.
  Plan: per patient, emit a cells x events binary matrix from the scDNA assay (so dropout and false
  positives are real), run SCITE, convert its `.gv` mutation tree to the
  `Patient_ID/Tree_ID/Node_ID/Mutation_ID/Parent_ID` layout, feed TreeMHN.
- **NEXT — clonealign's `L` is the true copy number.** `build_clonealign_inputs` derives the
  genes x clones matrix from iscc's true CN rather than from a CNV caller. Same class of problem;
  the fix is a copy-number-calling step (SCICoNE, or HMMcopy) upstream. SCICoNE needs packaging
  first — the author is doing that in a separate session (recommendation: bioconda/conda-forge
  recipe, plus a `shutil.which` fallback for `binary_path`).

## MHN cross-sectional cohort is degenerate (found 2026-08-26)

`notebooks/tool_mhn_bulk.ipynb` exists, runs in-process on the `py-mhn` kernel, and is deliberately
**not in the mkdocs nav**. On the `treemhn` cohort both planted parents (`E0`, `E3`) sit at frequency
**1.00**, because `event_size=8` was chosen so the accessibility-gated *children* occur at all. An
event present in every patient carries no cross-sectional information: Theta collapses to exactly
zero at every lambda tried from 3e-3 to 0.3, so it is unidentifiability, not over-regularisation.
The two observables want opposite regimes, so MHN needs its **own** cohort at lower `event_size`
(target: parents ~0.5-0.85, children in >=10 patients), generated by a new `_mhn` entry in
`make_analysis_data.py`. Both this and the SCITE work need a simulation run (grid 7, ~490 cells per
patient, so minutes) — pending user authorisation.
